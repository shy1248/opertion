...
"""
Usage:
  auto_task.py [options] cmd <command> [--skip-err] [--parallel]
  auto_task.py [options] put <src> <dst> [--parallel]
  auto_task.py [options] get <src> <dst>
...
""" 
by yushuibo 20160704 
update at 2017011,20170320,20171114
"""
...
def print_color(text, color=31, sep=' ', end='\n', file=stdout, flush=False):
	"""打印彩色字体,color默认为红色,该方法只针对Linux有效"""
	if platform.uname().system == 'Windows':
...
def get_ip_port(fname):
	"""从制定文件(特定格式)中，取得主机名/主机ip/端口; output:存储输出的对象"""
	try:
...
    """
	...
	# 据观察，下面三个变量的特点是"引用过一次"之后，其内容就会清空, 有readlines()的地方都是流，用过之后就没有了
...
def process_single_server(server_name, server_ip, port):
	"""处理一台server的逻辑"""
	# 可以看到多线程情况下，确实是不同的OutputText实例，说明threading.local()起到了预期作用
