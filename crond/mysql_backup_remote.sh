...
main(){
    ...
    backup
    # 
	local dest_tables=("tb_acc" "tb_agent" "tb_email" "tb_golbal" "tb_order_record" "tb_player")
	if [ x"$host" == x"10.0.0.14" ];then
    	backup df_chess_agent
		backup cxcc_chess_agent
		backup jpcc_chess_agent
		backup tdk_chess_agent
		backup ly_chess_agent
	fi
	if [ x"$host" == x"10.0.0.11" ];then
